...
class Rdt:
    ...
    # def addAmigo(self, name):
    #     self.amigos.append(name)
    ...
    def sendPkt(self, payload, seqNum): # Converte os dados em uma representação de string e codifica em bytes
        ...
        if randint(1,3):   #taxa de perda de 25%
            if self.type == "s": 
    ...
    def add_user(self, name, addr):
        ...
        self.payload = f"{name} entrou na sala"       
    ...
    def broadcast_ban(self):
        if self.counter >= (floor(len(self.users)/2)+1):
            msg = f"{self.counter}/{len(self.users)}"
            self.broadcast(msg)
            ...
            msg = f"[{self.banido}] saiu {self.counter}/{str(floor(len(self.users)/2)+1)}"
        self.broadcast(msg)
    ...
    def isSender(self, mensagem):
        self.time = 1
        self.rdt_socket.settimeout(self.time)
        #file_src = "alou.txt"  # Nome do arquivo a ser enviado
